...
const ClientTable = ({ tableData, refresh }) => {
    ...
    const handleModalOk = async (record) => {
        ...
        await axios.put('clients/' + record.id, record)
            .then(res => {
    ...
    return (
        <>
            <div className="py-12">
                <h1 className="text-sm w-48 left-0 relative right-0 ml-auto border-b-2 border-badge pb-1 text-right m-2">リクエスター総数　{dataSource?.length}名</h1>
